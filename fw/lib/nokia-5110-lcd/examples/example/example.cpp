...
int blPin = A0;
Nokia5110LCD::Display nokiaLCD(D0, D1, D2, blPin);
...
void loop()
{
	...
	if (Serial.available())
	{
		...
		case '~': // Use ~ to clear the screen.
			...
			break;
		default:
			nokiaLCD.setChar(c, cursorX, cursorY, BLACK);
			nokiaLCD.updateDisplay();
...
void lcdFunTime()
{
	nokiaLCD.clearDisplay(WHITE); // Begin by clearing the display
	...
	nokiaLCD.setStr("full of stars", 0, LCD_HEIGHT-8, BLACK);
	nokiaLCD.updateDisplay();
	delay(1000);
	// Seizure time!!! Err...demoing invertDisplay()
	for (int i=0; i<5; i++)
	{
		nokiaLCD.invertDisplay(); // This will swap all bits in our display
		delay(200);
		nokiaLCD.invertDisplay(); // This will get us back to where we started
		delay(200);
	}
	delay(2000);

	// setLine Example
	nokiaLCD.clearDisplay(WHITE); // Start fresh
	int x0 = LCD_WIDTH/2;
	int y0 = LCD_HEIGHT/2;
	for (float i=0; i<2*PI; i+=PI/8)
	{
		...
		nokiaLCD.setLine(x0, y0, x1, y1, BLACK);
		nokiaLCD.updateDisplay();
		delay(100);
	}
	...
	{ // Swipe right black
		nokiaLCD.setRect(0, 0, x, LCD_HEIGHT, 1, BLACK);
		nokiaLCD.updateDisplay();
		delay(10);
	}
	for (int x=0; x<LCD_WIDTH; x+=8)
	{ // Swipe right white
		nokiaLCD.setRect(0, 0, x, LCD_HEIGHT, 1, WHITE);
		nokiaLCD.updateDisplay();
		delay(10);
	}
	for (int x=0; x<12; x++)
	{ // Shutter swipe
		nokiaLCD.setRect(0, 0, x, LCD_HEIGHT, 1, 1);
		nokiaLCD.setRect(11, 0, x+12, LCD_HEIGHT, 1, BLACK);
		nokiaLCD.setRect(23, 0, x+24, LCD_HEIGHT, 1, BLACK);
		nokiaLCD.setRect(35, 0, x+36, LCD_HEIGHT, 1, BLACK);
		nokiaLCD.setRect(47, 0, x+48, LCD_HEIGHT, 1, BLACK);
		nokiaLCD.setRect(59, 0, x+60, LCD_HEIGHT, 1, BLACK);
		nokiaLCD.setRect(71, 0, x+72, LCD_HEIGHT, 1, BLACK);
		nokiaLCD.updateDisplay();
		delay(10);
	}
	// 3 Dee!
	nokiaLCD.setRect(25, 10, 45, 30, 0, WHITE);
	nokiaLCD.setRect(35, 20, 55, 40, 0, WHITE);
	nokiaLCD.setLine(25, 10, 35, 20, WHITE);
	nokiaLCD.setLine(45, 30, 55, 40, WHITE);
	nokiaLCD.setLine(25, 30, 35, 40, WHITE);
	nokiaLCD.setLine(45, 10, 55, 20, WHITE);
	nokiaLCD.updateDisplay();
	delay(2000);
	...
	for (int i=0; i<20; i++)
	{
		...
		nokiaLCD.setCircle(x, y, i, BLACK, 1);
		nokiaLCD.updateDisplay();
		delay(100);
	}
	delay(2000);
	...
	nokiaLCD.setStr("Modern Art", 0, 10, WHITE);
	nokiaLCD.updateDisplay();
	delay(2000);
	...
	// as our screen.
	nokiaLCD.setBitmap(xkcdSandwich);
	nokiaLCD.updateDisplay();
}
